...
export default function Vehiculos() {
  ...
  const [form, setForm] = useState({ placa: "", marca: "", modelo: "", anio: new Date().getFullYear(), tipo: "Camión", kilometraje: 0, soatVencimiento: "", tecnomecanicaVencimiento: "", estado: "ACTIVO", conductorId: "" });
  ...
  const handleCrear = async (e) => {
    e.preventDefault();
    try {
      await crearVehiculo({ ...form, anio: parseInt(form.anio), kilometraje: parseInt(form.kilometraje), conductorId: form.conductorId || null });
      setModalNuevo(false);
      setForm({ placa: "", marca: "", modelo: "", anio: new Date().getFullYear(), tipo: "Camión", kilometraje: 0, soatVencimiento: "", tecnomecanicaVencimiento: "", estado: "ACTIVO", conductorId: "" });
      cargar();
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full">
      <Header
        ...
        actions={
          <button onClick={handleAbrirModalNuevo} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}>
      ...
      >
        <form onSubmit={handleCrear} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>Conductor (opcional)</label>
            <select value={form.conductorId} onChange={(e) => setForm({ ...form, conductorId: e.target.value })} className="w-full px-3 py-2 rounded-lg border text-sm" style={{ backgroundColor: "var(--bg-input)", borderColor: "var(--border)" }}>
              ...
              {conductores.map((c) => (
                <option key={c.id} value={c.id}>{c.usuario?.nombre || c.cedula}</option>
              ))}
            </select>
          </div>
        </form>
